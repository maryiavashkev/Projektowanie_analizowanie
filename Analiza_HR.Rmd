...
Celem raportu jest analiza firmowych danych HR. Rotacja pracowników (Attrition) jest jednym z kosztowniejszych problemów dla działów HR. Koszt rekrutacji, wdrożenia i utraconej produktywności może wynosić od 50% do 200% rocznego wynagrodzenia pracownika. 

Celem tego raportu jest zidentyfikowanie kluczowych czynników wpływających na decyzję o odejściu (nadgodziny, wynagrodzenie, wiek, środowisko pracy), aby umożliwić firmie podjęcie działań prewencyjnych i zatrzymanie kluczowych talentów.
...
I -  Czy nadgodziny i niskie zadowolenie ze środowiska zwiększają ryzyko odejścia pracownika?
II - Czy pracownicy z niższym miesięcznym wynagrodzeniem lub rzadkimi awansami odchodzą częściej?
III -  Jaka jest charakterystyka (wiek, staż pracy) pracownika, który najczęściej decyduje się na odejście?
IV -  Które działy oraz stanowiska są najbardziej narażone na odejścia pracowników?
...
## Czy nadgodziny zwiększają ryzyko odejścia? (Pytanie badawcze nr 1)
```{r}
library(ggstatsplot)
...
## Czy pracownicy z niższym wynagrodzeniem odchodzą częściej? (Pytanie badawcze nr 2)
...
## Analiza odejść w zależności od Działu (Pytanie badawcze nr 4)
```{r}
ggbarstats(
  data = dane,
  x = Department,
